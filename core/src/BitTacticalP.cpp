...
namespace core {
    ...
    void BitTacticalP<T>::computeTacticalPTile(int batch, const std::vector<int> &list_act_x,
            ...
            const set_schedule &dense_schedule, int schedule_time, uint16_t act_mask,
    ...
    void BitTacticalP<T>::run(const base::Network<T> &network, const std::vector<schedule> &schedules) {
        ...
        std::string filename = "BitTacticalP_L" + std::to_string(this->N_LANES) + "_L" + std::to_string(this->N_COLUMNS)
                + "_R" + std::to_string(this->N_ROWS) + "_T" + std::to_string(this->N_TILES) + "_PG_" +
                ...
                std::to_string(this->LOOKASIDE_D) + ")" + (LEADING_BIT ? "_LB" : "") + "_cycles";
        ...
        for(auto layer_it = 0; layer_it < network.getLayers().size(); ++layer_it) {
            ...
            for(int n = 0; n < batch_size; n++) {
                ...
                if (conv) {
                    ...
                    for (int tile = 0; tile < this->N_TILES; tile++) {
                        uint64_t tile_cycles = *std::max_element(cycles_per_group[tile].begin(),
                                cycles_per_group[tile].end());
                        ...
                        for (int set = 0; set < dense_schedule.size(); set += this->N_TILES) {
                            ...
                            for (int tile = 0; tile < this->N_TILES; tile++) {
                                ...
                                for (int schedule_time = 0; schedule_time < set_dense_schedule.size(); schedule_time++) {
                                    ...
                                    auto column_cycles = computeTacticalPColumn(n, r, 0, 0 ,0, act, set_dense_schedule,
                                            schedule_time, act_mask, lstm);
