...
namespace core {
                ...
                for (int i = 0; i < Kx; i++) {
                    for (int j = 0; j < Ky; j++) {
                        for (int k = 0; k < act_channels; k += WEIGHT_LANES) {
                            stats.cycles[index][n] += computeStripesTile(list_x, list_y, i, j, layer_prec, k,
                                    act_channels, rowMap);
                        }
                    }
                }
    ...
    void Stripes<T>::computeInnerProduct(const Layer<T> &layer, sys::Statistics::Stats &stats) {
        ...
        for (n = 0; n<batch_size; n++) {
            for (int k = 0; k<act_channels; k += WEIGHT_LANES) {
                stats.cycles[index][n] += computeStripesColumn(0,0,0,0,layer_prec,k,act_channels,rowMap);
            }
            stats.cycles[index][n] *= num_filters_sets;
        }
