...
namespace core {
    ...
    uint16_t SCNNp<T>::computeSCNNpBitsPE(T act, T wgt, uint8_t act_layer_prec, const int network_bits) {
        ...
        #endif
        else return (uint16_t)(act_layer_prec * network_bits);
    }

    template <typename T>
    typename SCNNp<T>::PE_stats SCNNp<T>::computeSCNNpPE(uint64_t W, uint64_t H, int stride, const act_idxMap &act,
    ...
    template <typename T>
    typename SCNNp<T>::Tile_stats SCNNp<T>::computeSCNNpTile(int n, int ct, int ck, int kc, int tw, int th, uint64_t X,
    ...
    template <typename T>
    void SCNNp<T>::run(const base::Network<T> &network) {

        // Initialize statistics
        std::string filename = "SCNNp_Wt" + std::to_string(this->Wt) + "_Ht" + std::to_string(this->Ht) + "_I" +
            ...
            for(int n = 0; n < N; n++) {
                for(int kc = 0; kc < K; kc += Kc) {
                    ...
                    for(int ck = 0; ck < Ck; ck++) {
                        auto tile_stats = computeSCNNpTile(n,ct,ck,kc,tw,th,X,Y,Kc,K,W,H,R,S,stride,padding,act,wgt);
        ...
        //Dump statistics
        std::string header = "SCNNp Number of Cycles for " + network.getName() + "\n";
    ...
    template <typename T>
    void SCNNp<T>::potentials(const base::Network<T> &network) {

        // Initialize statistics
        std::string filename = "SCNNp_potentials";
        sys::Stats stats = sys::Stats(network.getNumLayers(), this->FAST_MODE ? 1 : network.getBatches(), filename);
        ...
        //Dump statistics
        std::string header = "SCNNp Potentials/Work Reduction for " + network.getName() + "\n";
        #ifdef ZERO_COUNT
