...
namespace core {
    ...
    template <typename T>
    void SCNNp<T>::computeSCNNpTile(int n, int ct, int ck, int kc, int tw, int th, uint64_t X, uint64_t Y, int Kc,
    ...
    template <typename T>
    void SCNNp<T>::computeSCNNpLayer(const core::Layer<T> &layer, sys::Statistics::Stats &stats) {
    ...
    template <typename T>
    void SCNNp<T>::run(const Network<T> &network) {
