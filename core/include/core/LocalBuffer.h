...
namespace core {
    ...
    public:
        ...
        LocalBuffer(const std::shared_ptr<std::map<uint64_t, uint64_t>> &_tracked_data, uint32_t _ROWS,
                uint32_t _READ_DELAY, uint32_t _WRITE_DELAY) : Memory<T>(_tracked_data), ROWS(_ROWS),
