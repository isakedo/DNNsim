...
namespace core {
    ...
    public:
        ...
        SCNNp(uint32_t _Wt, uint32_t _Ht, uint32_t _I, uint32_t _F, uint32_t _out_acc_size, uint32_t _BANKS,
                uint32_t _PE_SERIAL_BITS, uint8_t _N_THREADS, bool _FAST_MODE, bool _QUIET, bool _CHECK) : SCNN<T>(_Wt,
                _Ht,_I,_F,_out_acc_size,_BANKS,_N_THREADS,_FAST_MODE,_QUIET,_CHECK), PE_SERIAL_BITS(_PE_SERIAL_BITS) {}
